...
public class PermissionAuthorizationFilter extends OncePerRequestFilter {
    ...
    private void sendUnauthorizedResponse(HttpServletResponse response, String message) throws IOException {
        ...
        String errorResponse = String.format(
            "{\"success\":false,\"error\":{\"type\":\"AUTHENTICATION_ERROR\",\"message\":\"%s\"}}",
            message.replace("\"", "\\\"")
        );
    ...
    private void sendForbiddenResponse(HttpServletResponse response, String message) throws IOException {
        ...
        String errorResponse = String.format(
            "{\"success\":false,\"error\":{\"type\":\"AUTHORIZATION_ERROR\",\"message\":\"%s\"}}",
            message.replace("\"", "\\\"")
        );
        response.getWriter().write(errorResponse);
    }
    ...
    private void sendInternalErrorResponse(HttpServletResponse response, String message) throws IOException {
        ...
        String errorResponse = String.format(
            "{\"success\":false,\"error\":{\"type\":\"SYSTEM_ERROR\",\"message\":\"%s\"}}",
            message.replace("\"", "\\\"")
        );
        response.getWriter().write(errorResponse);
    }
