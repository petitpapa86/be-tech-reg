...
//    }
    ...
    public void publishAsReplay(DomainEvent event) {
        logger.info("Publishing domain event (outbox replay): {}", event.getClass().getSimpleName());
        java.lang.ScopedValue.where(com.bcbs239.regtech.core.infrastructure.context.CorrelationContext.OUTBOX_REPLAY, Boolean.TRUE)
                .where(com.bcbs239.regtech.core.infrastructure.context.CorrelationContext.CORRELATION_ID, event.getCorrelationId())
                .where(com.bcbs239.regtech.core.infrastructure.context.CorrelationContext.CAUSATION_ID, event.getEventId())
