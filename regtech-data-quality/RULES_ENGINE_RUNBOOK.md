...
- `dataquality.business_rules` - Rule definitions
- `dataquality.rule_parameters` - Rule parameters
- `dataquality.rule_violations` - Violation audit trail
- `dataquality.rule_execution_log` - Performance metrics
- `dataquality.rule_exemptions` - Approved exemptions
...
1. **Identify Current Value**
   ...
   JOIN dataquality.rule_parameters rp ON br.id = rp.rule_id
...
3. **Update Parameter**
   ...
   WHERE rule_id = (SELECT id FROM dataquality.business_rules WHERE rule_code = 'RULE_CODE_HERE')
...
2. **Create Exemption**
   ...
   INSERT INTO dataquality.rule_exemptions (
       ...
       (SELECT id FROM dataquality.business_rules WHERE rule_code = 'RULE_CODE_HERE'),
...
3. **Verify Revocation**
   ...
   FROM dataquality.rule_exemptions
   WHERE id = EXEMPTION_ID;
   ```
...
2. **Export Parameters**
   ...
   COPY (
       SELECT 
           ...
           rp.parameter_value,
           rp.parameter_type
       FROM dataquality.rule_parameters rp
       JOIN dataquality.business_rules br ON rp.rule_id = br.id
...
3. **Export Lists**
   ...
   COPY (
       SELECT 
           ...
           rl.list_name,
           rl.list_values
       FROM dataquality.rule_lists rl
       JOIN dataquality.business_rules br ON rl.rule_id = br.id
...
4. **If Parameter Issue:**
   ...
   WHERE rule_id = (SELECT id FROM dataquality.business_rules WHERE rule_code = 'RULE_CODE')
     AND parameter_name = 'PARAMETER_NAME';
   ```
...
**Check:**
1. Is rule enabled? `SELECT enabled FROM dataquality.business_rules WHERE rule_code = 'RULE_CODE';`
