...
public record Money(BigDecimal amount, Currency currency) {
    ...
    private Result<Void> validateCurrency(Money other) {
        if (!this.currency.equals(other.currency)) {
            return Result.failure(ErrorDetail.of("CURRENCY_MISMATCH", ErrorType.BUSINESS_RULE_ERROR,
                String.format("Cannot perform operation on different currencies: %s and %s", 
                    this.currency.getCurrencyCode(), other.currency.getCurrencyCode()), "money.currency.mismatch"));
        }
